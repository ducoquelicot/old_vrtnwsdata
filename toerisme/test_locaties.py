...
def extract_attractions(browser, wait):
    ...
    for row in atrr_output:
        ...
        # adres
        try:
            adres_output = review_soup.select('div.LjCWTZdN span')
            adres = adres_output[1].text
        except:
            adres = False
        ...
        locatie = [location, att, adres, latitude, longitude]
        with open('toerisme/locaties.csv', 'a') as adressen:
            writer = csv.writer(adressen)
            writer.writerow(locatie)
        print("Appending location to csv...")
